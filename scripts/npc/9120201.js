...
var event = "ShowaBattle";

function start() {
    if (cm.haveItem(expedItem)) {
        cm.sendYesNo("Do you wish to take on The Boss?");
    } else {
        cm.sendOk("Leader must have an #b#t" + expedItem + "##k to enter.");
        cm.dispose();
    }
}
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        var em = cm.getEventManager(event);
        if (em != null) {
