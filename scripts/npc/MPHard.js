...
function start() {
    if (cm.getPlayer().isGroup()) {
        if (cm.getPlayer().isLeader()) {
            cm.sendSimple("Welcome to Hard MP Boss PQ. Which boss would your group want to take on?\r\n\Each Battle costs 250 Stamina\r\n\#L1# Pianus#l\r\n\#L2# Papulatus#l\r\n\#L3# Zakum#l\r\n\#L4# Horntail#l\r\n\#L5# Scarga/Targa#l\r\n\#L6# BigBoss#l\r\n\#L7# Pink Bean#l\r\n\#L8# Von Leon#l\r\n\#L9# Ark#l\r\n\#L10# Empress#l\r\n");
        } else {
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (cm.getPlayer().isGroup()) {
            if (em != null) {
                ...
                if (eli.size() > 0) {
                    if (cm.getPlayer().getStamina() >= cost) {
                        if (!em.startPlayerInstance(cm.getPlayer(), 5)) {
            ...
            if (cm.getPlayer().getStamina() >= cost) {
                if (!em.startPlayerInstance(cm.getPlayer(), 4)) {
                    cm.sendOk("Someone is already attempting the PQ or your instance is currently being reseted. Try again in few seconds.");
                } else {
                    cm.getPlayer().removeStamina(cost);
                }
