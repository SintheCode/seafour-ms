    ...
    if (mode < 0)
        ...
        if (status == 0 && mode == 1) {
            ...
            if (option == 7) {//2340000 white scroll
                if (mPoint > 0) {
                    cm.sendGetText("Hello#b #h ##k, How many White Scrolls would you like?\r\n\r\n#kEach Scroll is worth 1 Maple Point.\r\n\#kYour currently have " + mPoint + " Maple Points that you can exchange for.\r\n\r\n");
                } else {
                    cm.sendOk("Sorry, you dont seem to have any maple points.");
                    cm.dispose();
                }
            ...
            if (option == 15) {//frenzy totem
                if (mPoint >= 100) {
                    cm.sendYesNo("Are you sure you want to spend 100 MP for Frenzy Totem (PERM)? This totem increases spawn amount and counts by 50%.");
                } else {
            ...
            if (option == 16) {//guardian totem
                if (mPoint >= 250) {
                    cm.sendYesNo("Are you sure you want to spend 250 MP for Guardin Totem (PERM)? This totem reduces all damage by 50%.");
                } else {
                    cm.sendOk("Sorry, you dont seem to have enough maple points.");
                    cm.dispose();
                }
            ...
            if (option == 17) {//raging totem
                if (mPoint >= 500) {
                    cm.sendYesNo("Are you sure you want to spend 1000 MP for Frenzy Totem (PERM)? This totem forces all monsters to spawn where totem is spawned.");
                } else {
                    cm.sendOk("Sorry, you dont seem to have enough maple points.");
                    cm.dispose();
                }
            ...
            if (option == 20) {//skill panel
                if (mPoint >= 10) {
                    cm.sendYesNo("Are you sure you want to spend 10 MP for Random Rank A Skill Panel?");
                } else {
                    cm.sendOk("Sorry, you dont seem to have enough maple points.");
                    cm.dispose();
                }
            ...
            if (option == 7) {
                amount = Number(cm.getText());
                if (mPoint >= amount) {
                    if (cm.canHold(2340000, amount)) {
                        cm.gainItem(2340000, amount);
                        cm.getPlayer().getCashShop().gainCash(2, -amount);
                        cm.sendOk("You have gained " + amount + " White Scrolls.");
                        cm.dispose();
                    } else {
                        cm.sendOk("Sorry, you do not have enough room in your inventory for this.");
                        cm.dispose();
            ...
            if (option == 13) {
                ...
                if (mPoint >= amount && amount > 0) {
                    cm.sendYesNo("Do you confirm that you want to spend " + amount + " Maple Points on " + (amount * 2) + " hours of VIP? \r\n\.");
                } else {
                    cm.sendOk("Sorry, you do not have enough maple points for this.");
                    cm.dispose();
                }
            ...
            if (option == 16) {
                if (cm.canHold(2003003, 1)) {
                    ...
                    cm.getPlayer().getCashShop().gainCash(2, -250);
            ...
            if (option == 17) {
                if (cm.canHold(2003002, 1)) {
                    ...
                    cm.getPlayer().getCashShop().gainCash(2, -500);
            ...
            if (option == 13) {
                cm.getPlayer().getCashShop().gainCash(2, -amount);
                cm.getPlayer().addVip(amount * 2);
                cm.sendOk("You have successfull added " + (amount * 2) + " hours of vip to your account");
                cm.dispose();
            }
            cm.dispose();
        } else {
