...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        if (selection == 1) {
            ...
            cm.sendNext("Items: \r\n\r\n\      Items dropped by monsters are based on tier and level. For example normal lvl 120 monster will drop an item with power of 120. For a tiered bosses, Tier 2 boss = 4x Power/Level, Tier 3 boss = 9x Power/Level, Tier 4 boss = 16x Power/Level, Tier 5 boss = 25x Power/Level, Tier 6 boss = 36x Power/Level. Stronger the monster Better the drops. Also drop rates increase with Tier/level. Certain bosses drop hidden bonus rewards.");
        } else if (selection == 4) {
            cm.sendNext("Scrolling: \r\n\r\n\      Scrolling items is EXTREMELY vital. Fully scroll lvl 30 can out perform lvl 180 with nothing. Each item comes with 1 slot, that slot is consumed once an item has reach +250. You can use a hammer-found from any boss to gain back the 1 slot and reset the items level without losing stats, think of if it as a reborned item. Chaos scrolls yield 0-5 stats on avg per scroll. Black Scroll consumes the slot and grants 0-250 base on remaining levels. Example would be if said item is +27 the black will grant 0-228 stats.");
        } else if (selection == 5) {
            ...
            cm.sendNext("Damage System: \r\n\r\n\      Monsters deal REAL damage shown on msg top middle of screen and red combo numbers to the rigt of screen. Damage does get reduced with Defense and Magic Defense properly. Higher tier monster ignore defense more and deal more damage.");
        }
    } else {
				status = 0;
				cm.sendSimple("Which option would you like to know more about?\r\n\#L1# Jobs #l\r\n\#L2# Monsters #l\r\n\#L3# Items #l\r\n\#L4# Scrolls #l\r\n\#L5# Custom Npcs #l\r\n\#L6# Custom Events #l\r\n\#L7# Damage System #l\r\n");
