...
import server.life.MonsterGlobalRankedDropEntry;
...
public class MapleMap {
    ...
    private static final int[] Maple11 = {1004492, 1102828, 1082647, 1052929, 1073057, 1132287, 1122306, 1302336, 1312201, 1322253, 1332277, 1372225, 1382263, 1402253, 1412180, 1422187, 1432216, 1442270, 1452255, 1462241, 1472263, 1482218, 1492233, 1012376};
    //Maps
    private static final int[] ToykoMaps = {240070100, 240070101, 240070102, 240070200, 240070201, 240070202, 240070203, 240070300, 240070301, 240070302, 240070303, 240070400, 240070401, 240070402, 240070403, 240070500, 240070501, 240070502, 240070503, 240070600, 240070601, 240070602, 240070603};
    public Point location;
    private boolean frenzytotem = false, ragingtotem = false, guardtotem = false;
    public int kills = 1;
    private static final int[] Elites = {8220022, 8220023, 8220024, 8220025, 8220026};
    public boolean locked = false;
    public int phase = 0;
    public int avgLevel;
    public MapleMonster totem;
    ...
    public MapleMap(int mapid, int world, int channel, int returnMapId, float monsterRate) {
        ...
        TimerManager.getInstance().register(new Runnable() {
            @Override
            public void run() {
                if (!getPlayerCount().isEmpty()) {
                    if (!getSpawnCount().isEmpty() && allowSummons()) {
                        respawn();
                    }
                }
            }
    ...
    public static int Maple11(int value) {
        return Maple11[value];
    }
    ...
    public void setFrenzyTotem(boolean toggle) {
        frenzytotem = toggle;
    }

    public boolean getFrenzyTotem() {
        return frenzytotem;
    }

    public void setRagingTotem(boolean toggle) {
        ragingtotem = toggle;
    }

    public boolean getRagingTotem() {
        return ragingtotem;
    }

    public void setGuardTotem(boolean toggle) {
        guardtotem = toggle;
    }

    public boolean getGuardTotem() {
        return guardtotem;
    }
    ...
    public Collection<SpawnPoint> getSpawnCount() {
        return monsterSpawn;
    }

    public Collection<MapleCharacter> getPlayerCount() {
        return characters;
    }
    ...
    public void removeAllMonsters() {
        objectWLock.lock();
        try {
            for (MapleMapObject obj : getMonsters()) {
                removeMapObject(obj.getObjectId());
            }
        } finally {
            objectWLock.unlock();
    ...
    public boolean toyko() {
        return this.getId() >= 82000 && this.getId() < 83000;
    }

    public boolean tower() {
        return this.getId() >= 90000 && this.getId() < 90100;
    }

    public boolean toggledrops(boolean value) {
        return dropsOn = value;
    }

    public boolean toggleconsume(boolean value) {
    ...
    private byte dropItemsFromMonsterOnMap(List<MonsterDropEntry> dropEntry, Point pos, byte d, double chRate, byte droptype, int mobpos, MapleCharacter chr, MapleMonster mob) {

        if (this.getId() == 8510100 || mob.dropsDisabled() || !dropsOn || dropEntry.isEmpty()) {
            return d;
        }
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (mob.getId() >= 9302021 && mob.getId() <= 9302038) {
            droptype = 3;
        }
        int rank = mob.getStats().getScale();
        List<MapleCharacter> players = chr.allMembersonSameMap();
        int range = (int) (25 + Math.pow(rank, 2));
        Item idrop;

        if ((chr.getParty() != null) || (chr.getRaid() != null && (chr.getEventInstance() != null))) {
            for (MapleCharacter player : players) {
                if (player.getDrops()) {
                    double rate = (player.getDropRate());
                    for (final MonsterDropEntry de : dropEntry) {
                        int dropChance = (int) (de.chance * rate * player.getCardRate(de.itemId));
                        if (Randomizer.nextInt(100000) <= dropChance) {
                            pos.x = (int) (player.getPosition().getX() + Randomizer.random(-range, range));
                            if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                                idrop = ii.randomizeStatsbyMobLevel((Equip) ii.getEquipById(de.itemId), (int) (mob.getStats().getPower() * player.lootDropChance() * (1 + player.itemR)));
                                if (player.recycle) {
                                    player.addEquipOverflow((Equip) idrop);
                                    continue;
                                }
                            } else {
                            ...
                            if (!MapleInventoryManipulator.addFromDrop(player.getClient(), idrop, false)) {
                                if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                                    ...
                                    int price = MapleItemInformationProvider.getInstance().getPrice(idrop.getItemId(), idrop.getQuantity());
                                    player.gainMeso(price, true, false);
                                    //spawnDrop(idrop, calcDropPos(chr.getPosition(), chr.getPosition()), mob, chr, droptype, de.questid);
                                    //d++;
                                }
                            }
                        }
            ...
            if (chr.getDrops() && chr.isAlive()) {
                double rate = (chr.getDropRate());
                for (final MonsterDropEntry de : dropEntry) {
                    int dropChance = (int) (de.chance * rate * chr.getCardRate(de.itemId));
                    if (Randomizer.nextInt(100000) <= dropChance) {
                        pos.x = (int) (chr.getPosition().getX() + Randomizer.random(-range, range));
                        if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                            idrop = ii.randomizeStatsbyMobLevel((Equip) ii.getEquipById(de.itemId), (int) (mob.getStats().getPower() * chr.lootDropChance() * (1 + chr.itemR)));
                            if (chr.recycle) {
                                chr.addEquipOverflow((Equip) idrop);
                                continue;
                            }
                        } else {
                            idrop = new Item(de.itemId, (short) 0, (short) (de.Maximum != 1 ? Randomizer.nextInt(de.Maximum - de.Minimum) + de.Minimum : 1));
    ...
    private byte dropGlobalItemsFromMonsterOnMap(List<MonsterGlobalDropEntry> globalEntry, Point pos, byte d, byte droptype, int mobpos, MapleCharacter chr, MapleMonster mob) {
        if (this.getId() == 8510100 || mob.dropsDisabled() || !dropsOn || !mob.getStats().getRevives().isEmpty()) {
            return d;
        }
        Item idrop;
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            ...
            for (MapleCharacter player : players) {
                if (player.getDrops()) {
                    ...
                    for (final MonsterGlobalDropEntry de : globalEntry) {
                        if (rank >= de.rank) {
                            if ((de.minlevel == 0 && de.maxlevel == 0) || (moblevel >= de.minlevel && moblevel < de.maxlevel)) {
                                int dropChance = (int) (de.chance * rate * player.getCardRate(de.itemId));
                                if (Randomizer.nextInt(100000) <= dropChance) {
                                    pos.x = (int) (player.getPosition().getX() + Randomizer.random(-range, range));
                                    if (ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean useBaseRate) {
        if (this.getDroppedItemCount() >= (ServerConstants.ITEM_LIMIT_ON_MAP)) {
            return;
        }
        if (mob.dropsDisabled() || !dropsOn || chr.getTested()) {
            return;
        }
        if (totem != null && totem.getId() == mob.getId()) {
            return;
        }

        final byte droptype = (byte) (mob.getStats().isExplosiveReward() ? 3 : mob.getStats().isFfaLoot() ? 2 : (chr.getParty() != null || chr.getRaid() != null) ? 1 : 0);

        double chRate = chr.getDropRate();
        Point pos = new Point(mob.getPosition().x, mob.getPosition().y);
        if (tower() && chr.getDrops()) {
            short amount = (short) Randomizer.randomMinMax(1, (int) (1 + ((mob.getChannel() * 0.25) - 0.25)));
            if (chr.getBuffedValue(MapleBuffStat.MESOUP) != null) {
                amount *= 2;
            }
            Item idrop = new Item(4310000, (short) 0, amount);
            if (!MapleInventoryManipulator.addFromDrop(chr.getClient(), idrop, false)) {
                pos.x = (int) (pos.x + Randomizer.random(-25, 25));
                spawnDrop(idrop, calcDropPos(pos, mob.getTruePosition()), mob, chr, droptype, (short) 0);
            }
        ...
        if (globalEntry.isEmpty() && dropEntry.isEmpty() && visibleQuestEntry.isEmpty()) {
            return;
        }
        registerMobItemDrops(droptype, mob.getPosition().x, chRate, pos, dropEntry, visibleQuestEntry, otherQuestEntry, globalEntry, chr, mob);
    }
    ...
    public void dropFromReactor(final MapleCharacter chr, final MapleReactor reactor, Item drop, Point dropPos, short questid) {
        spawnDrop(drop, this.calcDropPos(dropPos, reactor.getPosition()), reactor, chr, (byte) ((chr.getParty() != null || chr.getRaid() != null) ? 1 : 0), questid);
    }
    ...
    private void registerMobItemDrops(byte droptype, int mobpos, double chRate, Point pos, List<MonsterDropEntry> dropEntry, List<MonsterDropEntry> visibleQuestEntry, List<MonsterDropEntry> otherQuestEntry, List<MonsterGlobalDropEntry> globalEntry, MapleCharacter chr, MapleMonster mob) {
    ...
    public void removeItem(MapleMapObject item) {
        droppedItemCount.decrementAndGet();
        this.removeMapObject(item);
        unregisterItemDrop((MapleMapItem) item);
        this.broadcastMessage(MaplePacketCreator.removeItemFromMap(item.getObjectId(), 0, 0));
    }
    ...
    public final List<MapleMonster> getAllMonsters() {
        ...
        for (MapleMapObject mmo : getMonsters()) {
            ...
            if (mob.getId() != 1 || mob.getId() != 2 || mob.getId() != 3) {
                list.add(mob);
            }
        }
    ...
    public final List<MapleDoorObject> getAllDoors() {
        List<MapleDoorObject> list = new LinkedList<>();
        for (MapleMapObject mmo : getDoors()) {
            list.add((MapleDoorObject) mmo);
        }

        return list;
    }

    public final List<MapleMonster> getAllBosses() {
        List<MapleMonster> list = new LinkedList<>();
        for (MapleMapObject mmo : getMonsters()) {
            MapleMonster mob = (MapleMonster) mmo;
            if (mob.getStats().getScale() >= 3) {
                if (mob.getId() < 8610005 || mob.getId() > 8610027) {
                    list.add(mob);
                }
            }
        }
    ...
    public List<MapleCharacter> getAllPlayers() {
        ...
        try {
            ...
            Iterator<MapleCharacter> iter = character.iterator();
            while (iter.hasNext()) {
                if (iter.next().isGM()) {
                    iter.remove();
                }
    ...
    public void spawnZakum(int channel, int scale) {
        MapleMonster mob = MapleLifeFactory.getMonsterCustomRank(8800000, channel, scale, false);
        spawnFakeMonsterOnGroundBelow(mob, new Point(30, -220));
        for (int i = 8800003; i < 8800011; i++) {
            MapleMonster m = MapleLifeFactory.getMonsterCustomRank(i, channel, scale - 1, true);
            spawnMonsterOnGroundBelow(m, new Point(30, -220));
        }
    }
    ...
    public void killMonster(final MapleCharacter chr, final MapleMonster monster) {
        if (monster.getStats().getOwner() == chr) {
    ...
    public void broadcastPinkBeanVictory() {
        getWorldServer().dropMessage(6, "[Victory] In a swift stroke of sorts, the crew that has attempted Pink Bean has ultimately defeated it. The Temple of Time shines radiantly once again, the day finally coming back, as the crew that managed to finally conquer it returns victoriously from the battlefield!!");
    }

    private boolean removeKilledMonsterObject(MapleMonster monster) {
        spawnedMonstersOnMap.decrementAndGet();
        monster.aggroResetAggro();
        removeMapObject(monster.getObjectId());
    ...
    public void battleMusic() {
        if (getEventInstance() == null) {
            if (specialEvent) {
                changeMusic("GL_Masteria/Resurrection");
            } else {
                switch (phase) {
                    case 0:
                        changeMusic(getBGM());
                        break;
                    case 1:
                        changeMusic("BgmFF7/Fighting");
                        break;
                    case 2:
                        changeMusic("BgmFF8/Dont_be_Afraid");
                        break;
                    case 3:
                        changeMusic("BgmFF8/Man_with_Machine_Gun");
                        break;
                    case 4:
                        changeMusic("BgmFF8/Force_your_way");
                        break;
                    case 5:
                        changeMusic("BgmFF9/Evil_Messenger");
                        break;
                }
            }
        }
    ...
    public void battleMusic(MapleCharacter chr) {
        if (!monsterSpawn.isEmpty()) {
            if (specialEvent) {
                chr.changeMusic("GL_Masteria/Resurrection");
            } else {
                switch (phase) {
                    case 0:
                        chr.changeMusic(getBGM());
                        break;
                    case 1:
                        chr.changeMusic("BgmFF7/Fighting");
                        break;
                    case 2:
                        chr.changeMusic("BgmFF8/Dont_be_Afraid");
                        break;
                    case 3:
                        chr.changeMusic("BgmFF8/Man_with_Machine_Gun");
                        break;
                    case 4:
                        chr.changeMusic("BgmFF8/Force_your_way");
                        break;
                    case 5:
                        chr.changeMusic("BgmFF9/Evil_Messenger");
                        break;
                }
            }
        }
    ...
    public void specialZombieEvent() {
        if (!getAllMonsterSpawn().isEmpty()) {
            if (!specialEvent && !locked) {
                if (avgLevel == 0) {
                    avgLevel = 100;
                }
                specialEvent = true;
                killAllSpawnMonsters();
                battleMusic();
                ...
                eventTime = time;
                for (MapleCharacter chr : getAllPlayers()) {
                    chr.announce(MaplePacketCreator.getClock(getTimer()));
                }
    ...
    public void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, int animation) { //kaotic1
        if (monster == null) {
            return;
        }
        if (removeKilledMonsterObject(monster)) {
            if (monster.getId() == 1) {
                if (!getAllPlayers().isEmpty()) {
                    broadcastStringMessage(6, "Frenzy Totem has faded away.");
                }
            ...
            if (monster.getId() == 2) {
                if (!getAllPlayers().isEmpty()) {
                    broadcastStringMessage(6, "Raging Totem has faded away.");
                }
                setRagingTotem(false);
                setOwner(null);
            }
            if (monster.getId() == 3) {
                if (!getAllPlayers().isEmpty()) {
                    broadcastStringMessage(6, "Guardian Totem has faded away.");
                }
                setGuardTotem(false);
            }
            try {
                if (chr != null) {
                        ...
                        if (monster.getId() >= 8220022 && monster.getId() <= 8220026) {
                            for (MapleCharacter player : getAllPlayers()) {
                                if (player.isAlive()) {
                                    Item idrop;
                                    if (phase == 5) {
                                        idrop = new Item(Randomizer.random(3073000, 3073006), (short) 0, 1);//Rank S
                                    } else if (phase == 4) {
                                        idrop = new Item(Randomizer.random(3072000, 3072006), (short) 0, 1);//Rank A
                                    } else {
                                        idrop = new Item(Randomizer.random(3071000, 3071006), (short) 0, 1);//Rank B
                                    }
                                    if (!MapleInventoryManipulator.addFromDrop(player.getClient(), idrop, false)) {
                                        spawnDrop(idrop, calcDropPos(monster.getTruePosition(), monster.getTruePosition()), monster, player, (byte) 3, (short) 0);
                                    }
                                }
                            }
                            if (locked) {
                                ...
                                broadcastStringMessage(6, "Dark energy has faded away.");
                                battleMusic();
                            }
                        }
                        if (monster.getId() > 3) {
                            if (getId() != 5001) {
                                if (getEventInstance() == null && avgLevel >= 200) {
                                    if (!locked) {
                                        kills += 1;
                                        if (kills == 9000) {
                                            broadcastStringMessage(6, "You feel something in the dark energy.");
                                        }
                                        if (kills == 10000) {
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(Randomizer.random(8220022, 8220026), avgLevel, 3);
                                            spawnMonsterEffect(boss, 106, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            broadcastStringMessage(6, boss.getName() + " has appeared.");
                                            phase = 1;
                                            battleMusic();
                                        }
                                        if (kills == 24000) {
                                            broadcastStringMessage(6, "You really feel something in the dark energy.");
                                        }
                                        if (kills == 25000) {
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(Randomizer.random(8220022, 8220026), avgLevel, 4);
                                            spawnMonsterEffect(boss, 106, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            broadcastStringMessage(6, "Elite: " + boss.getName() + " has appeared.");
                                            phase = 2;
                                            battleMusic();
                                        }
                                        if (kills == 49000) {
                                            broadcastStringMessage(6, "You feel the dark energy swarming.");
                                        }
                                        if (kills == 50000) {
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(Randomizer.random(8220022, 8220026), avgLevel, 5);
                                            spawnMonsterEffect(boss, 106, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            broadcastStringMessage(6, "Super Elite: " + boss.getName() + " has appeared.");
                                            phase = 3;
                                            battleMusic();
                                        }
                                        if (kills == 99000) {
                                            broadcastStringMessage(6, "You feel the dark energy radianting everywhere.");
                                        }
                                        if (kills == 100000) {
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(Randomizer.random(8220022, 8220026), avgLevel, 6);
                                            spawnMonsterEffect(boss, 106, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            broadcastStringMessage(6, "Boss: " + boss.getName() + " has appeared.");
                                            phase = 4;
                                            battleMusic();
                                        }
                                        if (kills == 249000) {
                                            broadcastStringMessage(6, "You feel the dark energy rapidly radianting everywhere.");
                                        }
                                        if (kills == 250000) {
                                            locked = true;
                                            closeMapSpawnPoints();
                                            ...
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(Randomizer.random(8220022, 8220026), avgLevel, 7);
                                            spawnMonsterEffect(boss, 106, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            Server.getInstance().broadcastMessage(MaplePacketCreator.serverNotice(0, "Kaotic: " + boss.getName() + " has appeared at " + this.getMapName() + "."));
                                            kills = 0;
                                            phase = 5;
                                            battleMusic();
                                        }
                                ...
                                if (getEventInstance() == null && !locked) {
                                    if (!getMonsterSpawn().isEmpty()) {
                                        if (Randomizer.nextInt(10000) == 0) {
                                            List<SpawnPoint> randomSpawns = getMonsterSpawn();
                                            Collections.shuffle(randomSpawns);
                                            MapleMonster boss = MapleLifeFactory.getMonsterByLevel(9302038, avgLevel, 3);
                                            spawnMonsterEffect(boss, 124, randomSpawns.get(Randomizer.nextInt(randomSpawns.size())).getPosition());
                                            broadcastStringMessage(6, "A Golden Pig has suddenly appeared.");
                                        }
                                        if (Randomizer.nextInt(100000) == 1) {
                                            specialZombieEvent();
                                        }
                                    }
                                }
                    ...
                    if (buff > -1) {
                        ...
                        for (MapleMapObject mmo : this.getPlayers()) {
                            ...
                            if (character.isAlive()) {
                                ...
                                character.announce(MaplePacketCreator.showOwnBuffEffect(buff, 1));
                    ...
                    //combo
                    //if (chr == dropOwner && chr.getTotalCombo() < 9999) {
                    //    chr.setTotalCombo((short) (chr.getTotalCombo() + 1));
                    //}
                    //upgradeEquip(monster);
    ...
    public void upgradeEquip(MapleMonster monster, MapleCharacter player) {
        int rank = monster.getStats().getScale();
        double bonus = (monster.getLevel() * (1 * (monster.getChannel() - 1) * 0.5));
        if (player.isAlive()) {
            List<Short> item = new ArrayList<>();
            for (Item it : player.getInventory(MapleInventoryType.EQUIPPED).list()) {
                if (player.isEquip(it.getPosition())) {
                    item.add(it.getPosition());
                }
            }
            if (rank >= 5) {
                player.upgradeEquip(item.get(Randomizer.nextInt(item.size())), Randomizer.Min((int) (bonus / 5), 1), Randomizer.nextInt(4));
            }
            if (rank >= 4) {
                player.upgradeEquip(item.get(Randomizer.nextInt(item.size())), Randomizer.Min((int) (bonus / 25), 1), Randomizer.nextInt(4));
            }
        }
    }
    ...
    public void killAllMonsters(boolean toggle) {
        if (toggle) {
            closeMapSpawnPoints();
        }
    ...
    public void killAllSpawnMonsters() {
        for (MapleMonster monster : getAllMonsters()) {
            if (monster.getId() != 1 && monster.getId() != 2 && monster.getId() != 3) {
                killMonster(monster, null, false, 1);
            }
    ...
    public void removeReactors() {
        if (!getAllReactors().isEmpty()) {
            for (MapleReactor o : getAllReactors()) {
                removeMapObject(o);
            }
        }
    }
    ...
    public void removeAllObjects() {
        if (!getMapObjects().isEmpty()) {
            for (MapleMapObject o : getMapObjects()) {
                if (o.getType() == MapleMapObjectType.REACTOR || o.getType() == MapleMapObjectType.MONSTER) {
                    removeMapObject(o);
                }
            }
        }
    ...
    public List<MapleMapObject> getMapObjectsList() {
        objectRLock.lock();
        try {
    ...
    public void spawnMonsterOnGroundBelow(int id, int x, int y) {
        MapleMonster mob = MapleLifeFactory.getMonster(id, getChannelId());
        spawnMonsterOnGroundBelow(mob, new Point(x, y));
    }

    public void spawnMonsterOnGroundBelow(MapleMonster mob, Point pos, boolean loot) {
    ...
    public void spawnAllMonsterIdFromMapSpawnList(int id, int difficulty, boolean isPq) {
        for (SpawnPoint sp : getAllMonsterSpawn()) {
            if (sp.getMonsterId() == id && sp.shouldForceSpawn()) {
                spawnMonster(sp.getMonster(getChannelId()), difficulty, isPq);
            }
        }
    }
    ...
    public void spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos, MapleCharacter Owner) {
        monster.setMap(this);
        //monster.getStats().setOwner(Owner);
        Point spos = new Point(pos.x, pos.y - 1);
        spos = calcPointBelow(spos);
        if (spos == null) {
            return;
    ...
    public void spawnMonsterEffect(final MapleMonster monster, final int effect, Point pos) {
        monster.setMap(this);
        Point spos = new Point(pos.x, pos.y);
        if (getEventInstance() != null) {
            getEventInstance().registerMonster(monster);
        }
        monster.setPosition(spos);
    ...
    public void updateDoor(MapleCharacter chr) {
        for (MapleDoorObject doors : getAllDoors()) {
            if (chr.getParty() != null) {
                if (chr.getPartyMembersOnline().contains(chr.getParty().getLeader().getPlayer())) {
                    chr.addVisibleMapObject(doors);
                    return;
                }
            }
            if (chr.getRaid() != null) {
                if (chr.getRaidMembersOnline().contains(chr.getRaid().getLeader())) {
                    chr.addVisibleMapObject(doors);
                    return;
                }
            }
            if (chr == doors.getOwner()) {
                chr.addVisibleMapObject(doors);
                return;
            }
            chr.removeVisibleMapObject(doors);
        }
    }
    ...
    public void addPlayer(final MapleCharacter chr) {
        ...
        if (chr.getMonsterCarnival() != null) {
            chr.announce(MaplePacketCreator.getClock(chr.getMonsterCarnival().getTimeLeftSeconds()));
            if (isCPQMap()) {
                int team = -1;
                int oposition = -1;
                if (chr.getTeam() == 0) {
                ...
                chr.announce(MaplePacketCreator.startMonsterCarnival(chr, team, oposition));
            }
        }
        ...
        if (hasForcedEquip()) {
            chr.announce(MaplePacketCreator.showForcedEquip(-1));
        }
        if (specialEquip()) {
            chr.announce(MaplePacketCreator.coconutScore(0, 0));
        ...
        chr.announce(MaplePacketCreator.resetForcedStats());
        if (mapid == 914000200 || mapid == 914000210 || mapid == 914000220) {
            chr.announce(MaplePacketCreator.aranGodlyStats());
        }
        if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted()) {
            chr.announce(MaplePacketCreator.getClock((int) (chr.getEventInstance().getTimeLeft() / 1000)));
        }
        if (chr.getFitness() != null && chr.getFitness().isTimerStarted()) {
            chr.announce(MaplePacketCreator.getClock((int) (chr.getFitness().getTimeLeft() / 1000)));
        }

        if (chr.getOla() != null && chr.getOla().isTimerStarted()) {
            chr.announce(MaplePacketCreator.getClock((int) (chr.getOla().getTimeLeft() / 1000)));
        }
        ...
        if (hasClock()) {
            ...
            chr.announce((MaplePacketCreator.getClockTime(cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND))));
        }
        if (hasBoat() > 0) {
            if (hasBoat() == 1) {
                chr.announce((MaplePacketCreator.boatPacket(true)));
            } else {
                chr.announce(MaplePacketCreator.boatPacket(false));
            }
        }

        chr.receivePartyMemberHP();
        chr.receiveRaidMemberHP();
    ...
    public MaplePortal getRandomPlayerSpawnpoint() {
        ...
        if (!spawnPoints.isEmpty()) {
            for (MaplePortal port : getPortals()) {
                if (port.getType() >= 0 && port.getType() <= 1 && port.getTargetMapId() == 999999999) {
                    spawnPoints.add(port);
                }
            }
            portal = spawnPoints.get(new Random().nextInt(spawnPoints.size()));
        } else {
            portal = getPortal(new Random().nextInt(getPortals().size()));
        }
    ...
    public void broadcastDamage(MapleCharacter source, final byte[] packet, boolean repeatToSource) {
        broadcastDamage(repeatToSource ? null : source, packet, Double.POSITIVE_INFINITY, source.getPosition());
    }

    public void broadcastDamage(MapleCharacter source, final byte[] packet, boolean repeatToSource, boolean ranged) {
        broadcastDamage(repeatToSource ? null : source, packet, ranged ? getRangedDistance() : Double.POSITIVE_INFINITY, source.getPosition());
    }

    private void broadcastDamage(MapleCharacter source, final byte[] packet, double rangeSq, Point rangedFrom) {
        chrRLock.lock();
        try {
            for (MapleCharacter chr : characters) {
                if (chr != source) {
     ...
     * @param mobTime
     */
    public void addMonsterSpawn(MapleMonster monster, int mobTime, int team) {//kaotic
        Point newpos = calcPointBelow(monster.getPosition());
        newpos.y -= 1;
        SpawnPoint sp = new SpawnPoint(monster, newpos, !monster.isMobile(), mobTime, mobInterval, team, this.getChannelId());
        //System.out.println(channel);
        //System.out.println(this.getChannelId());
        monsterSpawn.add(sp);
        if (getEventInstance() != null) {
            spawnMonster(sp.getMonster(getChannelId()));//spawn bosses in events
        }
        if (mobTime == -1) {// -1 does not respawn and should not either but force ONE spawn
            spawnMonster(sp.getMonster(getChannelId()));
        }
    }
    ...
    private class MobLootEntry implements Runnable {
        ...
        protected MobLootEntry(byte droptype, int mobpos, double chRate, Point pos, List<MonsterDropEntry> dropEntry, List<MonsterDropEntry> visibleQuestEntry, List<MonsterDropEntry> otherQuestEntry, List<MonsterGlobalDropEntry> globalEntry, MapleCharacter chr, MapleMonster mob) {
    ...
    public void instanceMapFirstSpawn(int difficulty, boolean isPq) {
        for (SpawnPoint spawnPoint : getAllMonsterSpawn()) {
            if (spawnPoint.getMobTime() == -1) {   //just those allowed to be spawned only once
                spawnMonster(spawnPoint.getMonster(getChannelId()));
            }
        }
    }
    ...
    public boolean isBannedMap(int id) {
        switch (id) {
            case 78001:
            case 682000304:
            case 925020002:
    ...
    public boolean allowSummons() {
        return this.allowSummons;
    }

    public void getRandomSpawnPoint(MapleMonster monster) {
        if (!getMonsterSpawn().isEmpty()) {
            List<SpawnPoint> randomSpawns = getMonsterSpawn();
            int point = Randomizer.nextInt(randomSpawns.size());
            spawnMonsterWithEffect(monster, 15, randomSpawns.get(point).getPosition());
        } else {
            System.out.println("Error with spawn point on Map ID: " + this.getId());
        }
    }

    public MapleMonster getTotem() {
        return totem;
    }

    public boolean getTotemSpawned() {
        //System.out.println("count " + countMonster(1, 3));
        return frenzytotem || ragingtotem || guardtotem;
    }

    public boolean getRageTotemSpawned() {
        //System.out.println("count " + countMonster(1, 3));
        return ragingtotem;
    }

    public boolean getGuardTotemSpawned() {
        //System.out.println("count " + countMonster(1, 3));
        return guardtotem;
    }
    ...
    public int maxSpawnCount() {
        if (specialEvent) {
            return Randomizer.Max(getMonsterSpawn().size() * 6, 120);
        } else if (getFrenzyTotem()) {
            return Randomizer.Max(getMonsterSpawn().size() * 6, 120);
        } else if (getRagingTotem()) {
            return Randomizer.Max(getMonsterSpawn().size() * 8, 160);
        } else {
            return Randomizer.Max(getMonsterSpawn().size() * 4, 100);
        }
    }
    ...
    public void respawn() {
        if (!getMonsterSpawn().isEmpty() && !locked) {
            List<SpawnPoint> randomSpawns = getMonsterSpawn();
            Collections.shuffle(randomSpawns);
            int spawnAttempts = 0;
            int currentSpawn = 0;
            while (countMonsters() < maxSpawnCount()) {
                if (randomSpawns.get(currentSpawn).shouldSpawn()) {
                    if (!specialEvent) {
                        spawnMonsterOnGroundBelow(randomSpawns.get(currentSpawn).getMonster(getChannelId()), randomSpawns.get(currentSpawn).getPosition());
                    } else {
                        spawnMonsterOnGroundBelow(MapleLifeFactory.getMonsterByLevel(Randomizer.random(9400920, 9400923), avgLevel), randomSpawns.get(currentSpawn).getPosition());
                    }
                    spawnAttempts = 0;
                } else {
                    spawnAttempts++;
    ...
    public void instanceRespawn() {
        if (!getMonsterSpawn().isEmpty()) {
            List<SpawnPoint> randomSpawns = getMonsterSpawn();
            Collections.shuffle(randomSpawns);
            int currentSpawn = 0, spawnAttempts = 0;
            while (countMonsters() < maxSpawnCount()) {
    ...
    public void spawnHorntailOnGroundBelow(final Point targetPoint) {   // ayy lmao
        MapleMonster htIntro = MapleLifeFactory.getMonster(8810026, getChannelId());
        spawnMonsterOnGroundBelow(htIntro, targetPoint);    // htintro spawn animation converting into horntail detected thanks to Arnah

        final MapleMonster ht = MapleLifeFactory.getMonster(8810018, getChannelId());
        ...
        for (int x = 8810002; x <= 8810009; x++) {
            MapleMonster m = MapleLifeFactory.getMonster(x, getChannelId());
    ...
    public void spawnHorntailOnGroundBelow(int channel, int scale, final Point targetPoint) {   // ayy lmao
        MapleMonster htIntro = MapleLifeFactory.getMonster(8810026, getChannelId());
        spawnMonsterOnGroundBelow(htIntro, targetPoint);    // htintro spawn animation converting into horntail detected thanks to Arnah

        final MapleMonster ht = MapleLifeFactory.getMonsterCustomRank(8810018, channel, scale, true);
        ht.setParentMobOid(htIntro.getObjectId());
        ht.addListener(new MonsterListener() {
        ...
        for (int x = 8810002; x <= 8810009; x++) {
            MapleMonster m = MapleLifeFactory.getMonsterCustomRank(x, channel, scale, true);
            m.setParentMobOid(htIntro.getObjectId());
    ...
    public void setTrain(boolean flag) {
        this.training = flag;
    }

    public void lootmessage(double value, Equip item) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (value == 1.10) {
            broadcastStringMessage(5, "A Rare " + ii.getName(item.getItemId()) + " has dropped!");
        }
        if (value == 1.25) {
            broadcastStringMessage(5, "An Epic " + ii.getName(item.getItemId()) + " has dropped!");
        }
        if (value == 1.5) {
            broadcastStringMessage(6, "A Legendary " + ii.getName(item.getItemId()) + " has dropped!");
        }
    }
