...
function action(mode, type, selection) {
    if (mode < 0) {
        ...
        if (cm.getPlayer().getMapId() == 76102) {
            if (status == 0) {
                cm.sendYesNo("Would you like to leave here?");
            } else if (status == 1) {
                var map = cm.getPlayer().getSavedLocation("MONSTERPARK");
                if (map == -1) {
                    map = 100000000;
                }
            ...
            if (status == 0) {
                if (cm.getPlayer().getTotalLevel() >= 70) {
                    ...
                    cm.sendOk("You need to be a miniuim of lvl 70 to enter monster park.");
                    cm.dispose();
                }
            } else if (status == 1) {
                cm.sendNext("These contests involve #bsequential monster/boss fights#k, These will require some strategy time and enough supplies at hand, as they are not common fights.");
            } else if (status == 2) {
                ...
                cm.getPlayer().saveLocation("MONSTERPARK");
                cm.warp(76102, "sp");
                cm.dispose();
            }
        }
