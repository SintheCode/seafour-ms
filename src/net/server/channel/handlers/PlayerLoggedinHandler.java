...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        player.getMap().battleMusic(player);
        ...
        if (newcomer) {
            ...
            player.loadBuffs(player);
            player.vipCheck();
            player.getVIPDuration();
            player.setMasteries(player.getJobId());
            player.dropMessage(6, "Use @rank to display list of top players anytime.");

            if (Server.getInstance().getexprate()) {
                player.dropMessage(6, "[World Exp Rate] 2x with Double Exp event running.");
            } else {
                player.dropMessage(6, "[World Exp Rate] 1x.");
            }
            if (player.isGMJob()) {
                if (Server.getInstance().getdroprate()) {
                    player.dropMessage(6, "[World Drop Rate] 4x with Double Drop running and GM Drop Boost.");
                } else {
                    player.dropMessage(6, "[World Drop Rate] 2x with GM Drop Boost.");
                }
            } else {
                if (Server.getInstance().getdroprate()) {
                    player.dropMessage(6, "[World Drop Rate] 2x with x2 Double Drop running.");
                } else {
                    player.dropMessage(6, "[World Drop Rate] 1x.");
                }
            }
            c.announce(MaplePacketCreator.setNPCScriptable(ScriptableNPCConstants.SCRIPTABLE_NPCS));
            player.changeSkillLevel(SkillFactory.getSkill(10000000 * player.getJobType() + 12), (byte) (player.getLinkedLevel() / 10), 100, -1);
            player.customPanels();
        } else {
