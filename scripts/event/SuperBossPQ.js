...
function setup(player, diff) {
    var eim = em.newInstance("SuperBossPQ" + player.getName());
...
function playerEntry(eim, player) {
    ...
    player.dropMessage(6, "[Super Boss PQ] Event will begin in 10 seconds. Players: " + eim.getPlayers());
}
...
function start(eim) {
    ...
    var mob = MapleLifeFactory.getMonsterRankLink(startMobId, eim.getChannel(), 3, false);
    map.spawnMonsterWithEffect(mob, 15, new java.awt.Point(Randomizer.rand(-380, 1300), -10));
    eim.startEventTimer(eventTime * 60000);
    eim.dropMessage(6, "[Super Boss PQ] Battle has begun. Good Luck!!!");
    eim.schedule("bomb", 0);
}
...
function monsterValue(eim, mobId) {
    ...
    if (mobId >= startMobId && mobId < endMobId) {
        eim.getMapInstance(eventMapId).spawnMonsterWithEffect(MapleLifeFactory.getMonsterRankLink(mobId + 1, eim.getChannel(), 3, false), 15, new java.awt.Point(Randomizer.rand(-380, 1300), -10));
    }
    if (mobId == endMobId) {
