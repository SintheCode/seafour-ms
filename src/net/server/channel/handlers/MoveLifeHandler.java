    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (nextMovementCouldBeSkill) {
            ...
            if (noSkills > 0) {
                ...
                if (!(nextUse != null && monster.canUseSkill(nextUse, false) && nextUse.getHP() >= (int) (((float) monster.getHp() / monster.getMaxHp()) * 100))) {
        ...
        if (nextUse != null) {
            c.announce(MaplePacketCreator.moveMonsterResponse(objectid, moveid, mobMp, true, nextSkillId, nextSkillLevel));
        } else {
            c.announce(MaplePacketCreator.moveMonsterResponse(objectid, moveid, mobMp, true));
        }
        try {
            long movementDataStart = slea.getPosition();
            updatePosition(slea, monster, -2);
            long movementDataLength = slea.getPosition() - movementDataStart;
            ...
            System.out.println("Error monster movement");
        }
    }
