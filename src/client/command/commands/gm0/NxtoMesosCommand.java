...
public class NxtoMesosCommand extends Command {

    {
        setDescription("Converts 1 nx into 100 mesos. If maxxed mesos, remaining nx/mesos will be lost.");
    }

    @Override
    public void execute(MapleClient c, String[] params) {
        if (params.length < 1 || params[0] == null) {
            c.getPlayer().dropMessage(6, "Command is @nxtomesos ####.");
            return;
        }
        if (c.getPlayer().getLevel() >= 10) {
            ...
            if (value <= c.getPlayer().getCashShop().getCash(1) && value > 0) {
                ...
                c.getPlayer().gainMeso(value * 100);
                c.getPlayer().dropMessage(6, "You traded in " + value + " NX for " + value * 100 + " Mesos.");
