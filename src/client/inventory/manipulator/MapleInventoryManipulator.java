...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, int quantity) {
        return addById(c, itemId, quantity, null, -1, -1);
    }

    public static boolean addById(MapleClient c, int itemId, int quantity, long expiration) {
        return addById(c, itemId, quantity, null, -1, (byte) 0, expiration);
    }

    public static boolean addById(MapleClient c, int itemId, int quantity, String owner, int petid) {
        return addById(c, itemId, quantity, owner, petid, -1);
    }

    public static boolean addById(MapleClient c, int itemId, int quantity, String owner, int petid, long expiration) {
        return addById(c, itemId, quantity, owner, petid, (byte) 0, expiration);
    }

    public static boolean addById(MapleClient c, int itemId, int quantity, String owner, int petid, byte flag, long expiration) {
    ...
    private static boolean addByIdInternal(MapleClient c, MapleCharacter chr, MapleInventoryType type, MapleInventory inv, int itemId, int quantity, String owner, int petid, byte flag, long expiration) {
    ...
    private static boolean addFromDropInternal(MapleClient c, MapleCharacter chr, MapleInventoryType type, MapleInventory inv, Item item, boolean show, int petId) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            if (item.getItemId() == 4031865 || item.getItemId() == 4031866) {
                int nxGain = item.getItemId() == 4031865 ? 100 : 250;
                chr.getCashShop().gainCash(1, nxGain);
                chr.showHint("You have earned #e#b" + nxGain + " NX#k#n. (" + chr.getCashShop().getCash(1) + " NX)", 300);
                return true;
            }
            if (chr.applyConsumeOnPickup(item.getItemId())) {
                return true;
            }
            if (item.getItemId() == 4031868) {
    ...
    public static void removeFromSlot(MapleClient c, MapleInventoryType type, short slot, int quantity, boolean fromDrop) {
    ...
    public static void removeFromSlot(MapleClient c, MapleInventoryType type, short slot, int quantity, boolean fromDrop, boolean consume) {
        ...
        if (item != null) {
            ...
            System.out.println("Player: " + chr.getName() + " Using null items.");
        }
    }
            ...
            if (item != null) {
                if (item.getItemId() == itemId || item.getCashId() == itemId) {
                    if (removeQuantity <= item.getQuantity()) {
                        removeFromSlot(c, type, item.getPosition(), removeQuantity, fromDrop, consume);
                        removeQuantity = 0;
                        break;
                    } else {
    ...
    public static void move(MapleClient c, MapleInventoryType type, short src, short dst) {
        ...
        int oldsrcQ = source.getQuantity();
        int slotMax = ii.getSlotMax(c, source.getItemId());
    ...
    private static final short[] ringslots = {-16, -15, -13, -12};

    public static short ring(short value) {
        return ringslots[value];
    }
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if (source.getItemId() >= 1112000 && source.getItemId() < 1120000 && !ii.isCash(source.getItemId())) { //ring
            for (short slot : ringslots) {
                if (eqpdInv.getItem(slot) == null) {
                    dst = slot;
                }
            }
        }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, short src, int quantity) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (src < 0) {
        ...
        if (ItemConstants.isPet(itemId) || itemId == 1122156) {
            return;
        }
