...
public final class UseCashItemHandler extends AbstractMaplePacketHandler {
        ...
        if (itemType == 504) { // vip teleport rock
            ...
            if (player.getEventInstance() == null) {
                if (player.getStamina() >= 25) {
                    if (!vip) {
                        ...
                        if (victim != null) {
                            if (player.getEventInstance() == null) {
                                ...
                                player.dropMessage(1, "Player your trying to teleport to is inside an event.");
                            }
                        } else {
                    ...
                    player.dropMessage(1, "You must have at least 25 Stamina to use this rock.");
                }
            } else {
                player.dropMessage(1, "This cannot be used while inside an event map.");
            }
            ...
            switch ((itemId / 1000) % 10) {
                ...
                case 6: //item megaphone
                    ...
                    if (slea.readByte() == 1) { //item
                        ...
                        if (item == null) {
                            player.dropMessage(1, "Cannot use cash items in super mega.");
                            return;
                        }
            ...
            if (player.canHoldMeso(ii.getMeso(itemId))) {
                player.gainMeso(ii.getMeso(itemId), true, false, true);
                remove(c, position, itemId);
            } else {
                player.dropMessage(5, "You can't hold anymore mesos.");
            }
            ...
            if (player.getInventory(MapleInventoryType.CASH).findById(5570000) == null) {
                player.dropMessage("Hacking Hammers shame on you.");
                System.out.println("Player Name: " + player.getName() + " trying to ");
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            if (equip.getLevel() < 250) {
                player.dropMessage("Equip must be fully scrolled before hammer is applied.");
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            final Equip scrolled = (Equip) ii.scrollEquipWithId(toScroll, uitem.getItemId(), false, itemId, player.isGM(), player, false);
    ...
    private static void remove(MapleClient c, short position, int itemid) {
        if (itemid >= 5140000 && itemid <= 5140006) {
            return;
        }
