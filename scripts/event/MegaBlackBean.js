...
function setup(player, lobbyid) {
    var eim = em.newInstance("MegaBlackBean" + player.getName());
    var map = eim.getMapInstance(eventMapId);
    map.toggleconsume(false);
    map.killAllMonsters();
...
function start(eim) {
    ...
    var mob6 = MapleLifeFactory.getMonsterByLevelLink(9420620, eim.getAvgPlayerLevel(), 8, false);
...
function bean(eim) {
    ...
    if (eim.getIntProperty("finished") < 1) {
        if (eim.getMapInstance(eventMapId).getSpawnedMonstersOnMap() < 80) {
            for (var i = 1; i <= 20; i++) {
                map.spawnMonsterOnGround(MapleLifeFactory.getMonster(9430027, eim.getChannel()), new java.awt.Point(Randomizer.rand(-800, 800), Randomizer.rand(-40, -400)));
            }
        }
