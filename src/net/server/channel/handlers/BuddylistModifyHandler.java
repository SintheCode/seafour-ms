        ...
        if (mode == 1) { // add
                    ...
                    if (charWithId != null) {
                        ...
                        if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
                            ...
                            if (buddyAddResult == BuddyAddResult.ALREADY_ON_LIST && channel != -1) {
                                ...
                                try (Connection con = DatabaseConnection.getConnection();PreparedStatement ps = con.prepareStatement("INSERT INTO buddies (characterid, `buddyid`, `pending`) VALUES (?, ?, 1)")) {
                                    ps.setInt(1, charWithId.getId());
                                    ps.setInt(2, player.getId());
                                    ps.executeUpdate();
                                }
            ...
            if (!buddylist.isFull()) {
                try {
                    ...
                    if (otherChar == null) {
                        try (Connection con = DatabaseConnection.getConnection();PreparedStatement ps = con.prepareStatement("SELECT name FROM characters WHERE id = ?")) {
