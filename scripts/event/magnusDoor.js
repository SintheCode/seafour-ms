...
function setup(player, lobbyid) {
    var eim = em.newInstance("magnusDoor" + player.getName());
...
function start(eim) {
    ...
    map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880000, eim.getChannel()), new java.awt.Point(2800, -1350));
...
function boss(eim) {
    ...
    if (eim.getIntProperty("finish") < 1) {
        for (var i = 1; i <= Randomizer.rand(10, 20); i++) {
            map.spawnMonsterOnGround(MapleLifeFactory.getMonster(9833661, eim.getChannel()), new java.awt.Point(Randomizer.rand(450, 3000), Randomizer.rand(-1800, -1350)));
        }
        eim.schedule("boss", Randomizer.rand(30, 45) * 1000);
    }
}
