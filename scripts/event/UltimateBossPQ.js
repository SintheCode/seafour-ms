...
function setup(player, diff) {
    var eim = em.newInstance("UltimateBossPQ" + player.getName());
...
function playerEntry(eim, player) {
    ...
    player.dropMessage(6, "[Ultimate Boss PQ] Event will begin in 10 seconds. Players: " + eim.getPlayers());
}
...
function start(eim) {
    ...
    var mob = MapleLifeFactory.getMonsterRankLink(startMobId, eim.getChannel(), 6, false);
    map.spawnMonsterWithEffect(mob, 15, new java.awt.Point(Randomizer.rand(-380, 1300), -10));
    eim.startEventTimer(eventTime * 60000);
    eim.dropMessage(6, "[Ultimate Boss PQ] Battle has begun. Good Luck!!!");
    eim.schedule("bomb", 0);
}
...
function monsterValue(eim, mobId) {
    ...
    if (mobId >= startMobId && mobId < endMobId) {
        eim.getMapInstance(eventMapId).spawnMonsterWithEffect(MapleLifeFactory.getMonsterRankLink(mobId + 1, eim.getChannel(), 6, false), 15, new java.awt.Point(Randomizer.rand(-380, 1300), -10));
    }
    if (mobId == endMobId) {
        eim.setIntProperty("finished", 1);
        eim.gainPartyItem(4000313, 250);
        eim.victory(exitMap);
    }
    return 1;
}
