...
function setup(player, scale) {
    var eim = em.newInstance("MP_Papulatus" + player.getName());
...
function start(eim) {
    var map = eim.getMapInstance(eventMapId);
    map.spawnMonsterWithEffect(MapleLifeFactory.getMonsterCustomRank(9300214, eim.getChannel(), eim.getIntProperty("scale"), false), 15, new java.awt.Point(-408, -385));
...
function monsterKilled(mob, eim) {
    var map = eim.getMapInstance(eventMapId);
    if (mob.getId() == 9300214) {
        map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonsterCustomRank(8500002, eim.getChannel(), eim.getIntProperty("scale"), false), mob.getPosition());
    }
    if (mob.getId() == BossId) {
