...
public final class InventorySortHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (curr - c.getPlayer().getitemcooldown() >= cooldown) {
            ...
            c.announce(MaplePacketCreator.enableActions());
            c.getPlayer().saveCharToDBType(11);
        } else {
            c.getPlayer().dropMessage(6, "Time until next available item sorting " + ((long) (cooldown - (curr - c.getPlayer().getitemcooldown())) / 1000) + " seconds.");
            c.announce(MaplePacketCreator.enableActions());
        }
