...
function setup(player, diff) {
    var eim = em.newInstance("MegaBossPQ" + player.getName());
...
function playerEntry(eim, player) {
    ...
    player.dropMessage(6, "[Mega Boss PQ] Event will begin in 10 seconds. Players: " + eim.getPlayers());
}
...
function start(eim) {
    ...
    eim.dropMessage(6, "[Mega Boss PQ] Battle has begun. Good Luck!!!");
    eim.schedule("bomb", 0);
}
...
function monsterValue(eim, mobId) {
    ...
    if (mobId >= startMobId && mobId < endMobId) {
        eim.getMapInstance(eventMapId).spawnMonsterWithEffect(MapleLifeFactory.getMonsterRankLink(mobId + 1, eim.getChannel(), 4, false), 15, new java.awt.Point(Randomizer.rand(-380, 1300), -10));
    }
    if (mobId == endMobId) {
