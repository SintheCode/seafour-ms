    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (!chr.isAlive() && ServerConstants.USE_BUYBACK_SYSTEM) {
            ...
            if (chr.getEventInstance() != null) {
                c.announce(MaplePacketCreator.serverNotice(5, "Fast Travel to Free market is not allowed here."));
            ...
            if (FieldLimit.CANNOTMIGRATE.check(chr.getMap().getFieldLimit())) {
                chr.dropMessage(1, "You can't enter the Free Market from here.");
                c.announce(MaplePacketCreator.blockedMessage2(5));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            if (chr.getLevel() < 8) {
                chr.dropMessage(1, "You must be level 8 or greater to access FM.");
                c.announce(MaplePacketCreator.blockedMessage2(5));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            if (chr.getMapId() >= 81000 && chr.getMapId() != 910000000) {
            ...
            if (ServerConstants.USE_MTS) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        if (rs.getInt("type") != 1) {
                            Item i = new Item(rs.getInt("itemid"), (short) 0, rs.getInt("quantity"));
