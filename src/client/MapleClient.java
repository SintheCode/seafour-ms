...
public class MapleClient {
    ...
    public boolean hasBannedHWID() {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM hwidbans WHERE hwid LIKE ?")) {
                ps.setString(1, hwid);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs != null && rs.next()) {
                        if (rs.getInt(1) > 0) {
    ...
    public int login(String login, String pwd, String nibbleHwid) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT id, password, gender, banned, pin, pic, characterslots, tos, language FROM accounts WHERE name = ?")) {
    ...
    public void updateHWID(String newHwid) {
        ...
        if (split.length > 1 && split[1].length() == 8) {
            ...
            try (Connection con = DatabaseConnection.getConnection()) {
                try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET hwid = ? WHERE id = ?")) {
    ...
    public void checkChar(int accid) {//kaotic
        for (MapleCharacter chr : Server.getInstance().getWorld(0).getPlayerStorage().getAllCharacters()) {
            if (accid == chr.getAccountID()) {
                System.out.println("Player:  " + chr.getName() + " has been removed from world. Possible Dupe attempt.");
                chr.getClient().forceDisconnect();
                Server.getInstance().getWorld(0).getPlayerStorage().removePlayer(chr.getId());
            }
        }
    }
    ...
    public final void forceDisconnect() {
        if (canDisconnect()) {
            System.out.println("Client was disconnected: " + this.getAccountName());
                        ...
                        if (!this.serverTransition) { // meaning not changing channels
                            ...
                            if (player.getRaid() != null) {
                                if (player.getRaid().getLeader() == player) {
                                    player.getRaid().assignNewLeader();
                                }
                                if (player.getRaid() != null) {
    ...
    public int getVotePoints() {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT `votepoints` FROM accounts WHERE id = ?")) {
                ps.setInt(1, accId);
                try (ResultSet rs = ps.executeQuery()) {
    ...
    //public synchronized void announce(final byte[] packet) {     // thanks GitGud for noticing an opportunity for improvement by overcoming "synchronized announce"
    ...
    public void changeChannel(int channel) {
        player.saveCharToDBType(11);
        player.saveCharToDBType(12);
        if (player.getPlayerDoor() != null) {
            player.cancelMagicDoor();
        }
        ...
        try {
            announce(MaplePacketCreator.getChannelChange(this, InetAddress.getByName(socket[0]), Integer.parseInt(socket[1])));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
