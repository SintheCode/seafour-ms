...
import server.gachapon.GachaponProvider;
...
public class AbstractPlayerInteraction {
    ...
    public void warpParty(int id, int portalId, int fromMinId, int fromMaxId) {
        for (MapleCharacter mc : getPartyMembers()) {
            if (mc.getMapId() >= fromMinId && mc.getMapId() <= fromMaxId) {
                mc.announce(MaplePacketCreator.playPortalSound());
                mc.changeMap(id, portalId);
            }
        }
    }

    public void warpExitParty(int id) {
        int mapid = getMapId();
        for (MapleCharacter mc : getPartyMembers()) {
            if (mc.getMapId() >= mapid && mc.getMapId() <= mapid) {
    ...
    public void getETCItemName(int id) {
        Item item = getPlayer().getInventory(MapleInventoryType.ETC).findById(id);
    ...
    public void gainItem(int id, int quantity, boolean show) {//this will fk randomStats equip :P
    ...
    public Item gainItem(int id, int quantity, boolean randomStats, boolean showMessage) {
    ...
    public Item gainItem(int id, int quantity, boolean randomStats, boolean showMessage, long expires, MaplePet from) {
        Item item = null;
        MaplePet evolved;
        int petId = -1;
            ...
            if (ItemConstants.getInventoryType(id).equals(MapleInventoryType.EQUIP)) {
                ...
                if (item != null) {
                    ...
                    if (eqp.getUpgradeSlots() > 0) {
                        eqp.setUpgradeSlots(1);
                    } else {
                        eqp.setUpgradeSlots(0);
                    }
                }
            } else {
    ...
    public void gainEquip(int id, int level) {
        gainEquip(id, (short) 1, true, true, -1, null, level);
    }

    public void gainRareEquip(MapleCharacter player, int id, int level, int tier) {
        String msg = player.getName() + " : [Tier " + tier + "]: congratulations on your achievement!";
        Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.itemMegaphone(msg, false, c.getChannel(), gainEquip(id, (short) 1, true, true, -1, null, level)));
    }

    public void gainRareEquip(MapleCharacter player, int id, int level, int tier, String txt) {
        String msg = player.getName() + " : [Tier " + tier + "]: " + txt;
        Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.itemMegaphone(msg, false, c.getChannel(), gainEquip(id, (short) 1, true, true, -1, null, level)));
    }

    public void rewardEquip(MapleCharacter player, Item item, int level, int tier, String txt) {
        String msg = player.getName() + " : [Tier " + tier + "]: " + txt;
        Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.itemMegaphone(msg, false, c.getChannel(), item));
    }

    public void resetItem(int slot, int level) {
        Item item = getInventory(MapleInventoryType.EQUIP).getItem((byte) slot);
        getInventory(MapleInventoryType.EQUIP).removeItem((short) slot);
        gainEquip(item.getItemId(), level);
    }

    public Item getEquipbySlot(int slot) {
        return getInventory(MapleInventoryType.EQUIP).getItem((byte) slot);
    }

    public Item getEquippedbySlot(int slot) {
        return getInventory(MapleInventoryType.EQUIPPED).getItem((byte) slot);
    }

    public Item getUsebySlot(int slot) {
        return getInventory(MapleInventoryType.USE).getItem((byte) slot);
    }
    ...
    public Item gainEquip(int id, short quantity, boolean randomStats, boolean showMessage, long expires, MaplePet from, int level) {
        ...
        if (quantity >= 0) {
            ...
            if (ItemConstants.getInventoryType(id) == MapleInventoryType.EQUIP) {
                if (randomStats) {
                    MapleInventoryManipulator.addFromDrop(c, ii.randomizeStatsbyMobLevel((Equip) item, level), false, petId);
                } else {
                    MapleInventoryManipulator.addFromDrop(c, (Equip) item, false, petId);
                }
            } else {
    ...
    public void giveCharacterExp(long amount, MapleCharacter chr) {
        chr.gainExp((long) (amount * chr.getExpRate()), true, true);
    }

    public void givePartyExp(long amount, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            giveCharacterExp(amount * party.size(), chr);
        }
    }
    ...
    public void removeItem(short type, short slot, short amount) {
        ...
        MapleInventoryManipulator.removeById(c, tempItem.getInventoryType(), tempItem.getItemId(), -amount, false, false);
    }
    ...
    public void spawnMonster(int id, int x, int y) {
        MapleMonster monster = MapleLifeFactory.getMonster(id, c.getChannel());
        monster.setPosition(new Point(x, y));
        getPlayer().getMap().spawnMonster(monster);
    }

    public MapleMonster getMonsterLifeFactory(int mid) {
        return MapleLifeFactory.getMonster(mid, c.getChannel());
    }
    ...
    public void npcTalk(int npcid, String message) {
        c.announce(MaplePacketCreator.getNPCTalk(npcid, (byte) 0, message, "00 00", (byte) 8));
    }
    ...
    public List<Item> globalItemsFromGach(int townId, MapleCharacter chr, int cost, int count) { //amount of tickets used
        List<Item> rewards = GachaponProvider.getRewards(townId, cost, count);
    ...
    public void givePartyGachItems(int townId, List<MapleCharacter> party, int cost, int count) {
        for (MapleCharacter chr : party) {
            globalItemsFromGach(townId, chr, cost, count);
        }
    }

    public MapleCharacter getPlayerByName(String name) {
        return c.getWorldServer().getPlayerStorage().getCharacterByName(name.toLowerCase());
    }
