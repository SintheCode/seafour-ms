...
public class Rank {
    ...
    public static int getMobRank(int id) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT rank FROM mobdata WHERE mobid = ?")) {
                ps.setInt(1, id);
    ...
    public static int getMobLevel(int mid) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT level FROM mobdata WHERE mobid = ?")) {
                ps.setInt(1, mid);
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
